...
public static class Utilities
{
    private static readonly Func<Projectile, float> getArcHeightFactor =
        (Func<Projectile, float>)AccessTools.PropertyGetter(typeof(Projectile), "ArcHeightFactor").CreateDelegate(typeof(Func<Projectile, float>));

    private static readonly Func<Projectile, float> getDistanceCoveredFraction = (Func<Projectile, float>)
        AccessTools.PropertyGetter(typeof(Projectile), "DistanceCoveredFraction").CreateDelegate(typeof(Func<Projectile, float>));
    ...
    {
        return new(cells.Select(cell => new GlobalTargetInfo(cell, map)));
    }
    ...
    public static Action DeserializeAction(this string str)
    {
        ...
        return (Action)Type.GetType(type)?.GetMethod(method, AccessTools.all)?.CreateDelegate(typeof(Action));
    }
